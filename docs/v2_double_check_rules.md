...
2026-09-23 核對十份原始檔的 SHA256，均與 [inventory](../daily_auto/reference/inventory.json) 相符。本輪重新抽取競賽辦法 p7–10，並核對原始指南、schema、既有分頁抽取及程式。本文列的是規則覆蓋與測試位置；測試執行結果及實際回測數字以本版驗證報告為準。舊檔保留，不以改寫舊報告掩蓋口徑差異。
...
實作入口：[新帳本](../src/double_check_ledger.py)、[新選參器](../src/double_check_tuning.py)、[語意驗證器](../daily_auto/validate.py)、[超限歷史](../daily_auto/compliance_state.py)。對應測試：[新帳本反例](../tests/test_double_check.py)、[每日語意](../tests/test_daily_auto.py)、[超限交易日](../tests/test_daily_compliance_state.py)。各條測試並非完整性證明，須連同逐日實際輸出驗證。
...
營運與證據測試見 [operations tests](../tests/test_daily_auto_operations.py)。本地預留次數不是提交；HTTP 成功不是平台接受；accepted D-Plan 不是已完成結算。每個環節須核對 exact bytes、日期、隊伍、官方來源與收件時間。
...
[本版每日入口](../daily_auto/double_check.py)的 `plan` 即使研究稽核通過，仍回傳 `BLOCK_SUBMISSION`，不輸出 D-Plan。`research-plan` 僅保存 `RESEARCH-DRAFT_…json`，將候選放進 `candidate_plan` 包裝，頂層標記 `RESEARCH_DRAFT_NEVER_SUBMIT`；這不是官方可接收的 D-Plan。[每日入口測試](../tests/test_double_check_daily.py)覆蓋強制阻擋、研究草稿包裝、參數及原始碼遭修改等情境。
