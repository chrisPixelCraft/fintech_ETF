...
- **已解決（U1）：** 以 10/26 為首日
  ...
  - 休市表列 10/26 補假
  - 休市時自動跳過
  - 見 [UNRESOLVED 總表](#unresolved-總表) U1
...
| ID | 問題 | 安全預設 |
|---|---|---|
| U1 | 首個交易日 | **以 10/26 為準**：主辦方指定首日 10/26（辦法範例「10/25 19:30～10/26 08:55 繳交」）；證交所休市表 10/26 為補假。`contest_start` 設 10/26，兩種情況計數都正確；10/26 照常產生並上傳 D-Plan |
