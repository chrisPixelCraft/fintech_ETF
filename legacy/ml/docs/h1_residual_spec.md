...
| 階段 | 配對平均 Δ | 中位數 Δ | 95% CI | 勝率 |
|---|---:|---:|---:|---:|
| DEV 2015–2021（調參） | +1.17% | +0.70% | +0.71% ~ +1.64% | 62% |
| Validation 2022–2024（sanity，不乾淨） | +1.78% | +2.85% | +0.97% ~ +2.56% | 67% |
| **Test 2025-01 到 2026-09（決定性）** | **−2.15%** | **−2.20%** | **−3.81% ~ −0.49%** | **30%** |

- Test 四項中三項不通過 → **STOP，Mom20 維持**；依第 5 節不改網格、不換門檻、不再救
- Mom20 在 test 的 +8.79% 與 2025–2026 final test 完全一致，比較基準無誤
- 與第 6 節的事先風險一致：長窗口動能（Mom60 −1.78%）在 2025–2026 輸給 Mom20，扣除 beta 沒有改變這點
- 看過 validation 後才挑出的訊號，在沒看過的資料上反轉；2022–2024 的 +2.02% 不能當成證據
