#!/usr/bin/env python3
"""Independent read-only verification of the bounded v3 expansion."""
...
from scripts.verify_24d import verify_hash_map,verify_group,same_frame
...
def verify(output,rebuild=False):
    ...
    parent_seal=read(parent/'result_manifest.json')
    for name,checksum in guard['selection_source_sha256'].items():
        require(parent_seal['files'].get(str(Path(name).relative_to('outputs/24d_tuning')))==checksum,'Selection source is not sealed by previous result')
    ...
    old_ids={s['candidate_id'] for s in read(parent/'candidates.json')}
    ...
    require(members.family.value_counts().to_dict()=={'uniform':128,'availability':128,'joint_local':72},'Family count differs from frozen budget')
    ...
    require(freeze['prior_leader_id']==old_dev.iloc[0].candidate_id,'Prior leader not chosen from development')
