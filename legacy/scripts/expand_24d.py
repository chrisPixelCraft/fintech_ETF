#!/usr/bin/env python3
"""Development-only feasibility expansion; immutable prior studies are references."""
...
STUDY=ROOT/'config/24d_expansion_study.json'
PARENT=ROOT/'outputs/24d_tuning'
OUTPUT=ROOT/'outputs/24d_expansion'


def generate_candidates():
    previous=json.loads((PARENT/'candidates.json').read_text())
    lookup={p['candidate_id']:p for p in previous}
    ordered=pd.read_csv(PARENT/'development_ranking.csv')
    parents=[lookup[c] for c in ordered.candidate_id.head(2)]
    seen=set(lookup); candidates=[]; memberships=[]
    rng=np.random.default_rng(2409202602)
    for family in ['uniform','availability']:
        count=0
        while count<128:
            ...
            if family=='availability':
                for key,values in dict(volume_low=[.2,.5],volume_high=[5.,8.],volatility_spike_ratio=[3.,4.],one_day_chase_return=[.07,.085,.10]).items():
            ...
            memberships.append(dict(candidate_id=trial['candidate_id'],family=family,parent_id=None));count+=1
    for parent in parents:
        for n,cash,volume,volatility in itertools.product([20,25,30],[.08,.12,.18],[.2,.8],[2.,4.]):
            trial=spec({**parent['params'],'target_count':n,'cash_guard_ratio':cash,
                'volume_low':volume,'volatility_spike_ratio':volatility,'volume_high':8.,'one_day_chase_return':.10})
            if trial['candidate_id'] in seen:continue
            seen.add(trial['candidate_id']);candidates.append(trial)
            memberships.append(dict(candidate_id=trial['candidate_id'],family='joint_local',parent_id=parent['candidate_id']))
    references=unique([spec(),lookup[ordered.candidate_id.iloc[0]]])
...
def context(output):
    ...
    inputs=[PARENT/'candidates.json',PARENT/'development_ranking.csv']
...
def execute(output=OUTPUT,workers=4):
    ...
    freeze=dict(status='SELECTED_RESEARCH_ONLY' if selected else 'NO_ELIGIBLE_CANDIDATE',formal_status='BLOCK_SUBMISSION',
        ...
        candidate_count=len(candidates),previous_candidate_count=247,prior_holdout_exposure=True,
