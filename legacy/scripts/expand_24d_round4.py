#!/usr/bin/env python3
"""Fourth targeted daily-parameter feasibility search; previous sealed studies remain immutable."""
...
STUDY=ROOT/'config/24d_round4_study.json'
PARENT=ROOT/'outputs/24d_round3'
PARENTS=[ROOT/'outputs'/name for name in ['24d_tuning','24d_expansion','24d_round3']]
TUNED=['return_short','return_long','momentum_weight','long_return_fraction','max_replacements_per_day','replacement_margin']
OUTPUT=ROOT/'outputs/24d_round4'


def generate_candidates():
    previous=[trial for folder in PARENTS for trial in json.loads((folder/'candidates.json').read_text())]
    lookup={trial['candidate_id']:trial for trial in previous}
    if len(lookup)!=1087:raise ValueError('Expected 1087 prior distinct candidates')
    parents=[]
    for folder in PARENTS:
        own={trial['candidate_id'] for trial in json.loads((folder/'candidates.json').read_text())}
        ranking=pd.read_csv(folder/'development_ranking.csv')
        parents.append(lookup[ranking[ranking.candidate_id.isin(own)].candidate_id.iloc[0]])
    rng=np.random.default_rng(2409202604);seen=set(lookup);candidates=[];members=[]
    for folder,parent in zip(PARENTS,parents):
        count=0
        while count<128:
            params={**parent['params'],**{key:SPACE[key][int(rng.integers(len(SPACE[key])))] for key in TUNED}}
            ...
            members.append(dict(candidate_id=trial['candidate_id'],family=folder.name,parent_id=parent['candidate_id']));count+=1
    return candidates,unique([spec()]+parents),pd.DataFrame(members)
...
def context(output):
    ...
    inputs=[folder/name for folder in PARENTS for name in ['candidates.json','development_ranking.csv']]
    guard=dict(parent_guard=ctx['guard_sha256'],implementation={str(p.relative_to(ROOT)):engine.file_hash(p) for p in files},
        parent_result_manifest_sha256={str(folder.relative_to(ROOT)):engine.file_hash(folder/'result_manifest.json') for folder in PARENTS},
...
def seal(out,freeze):
    ...
    engine.atomic_json(out/'result_manifest.json',dict(status='COMPLETE',formal_status='BLOCK_SUBMISSION',
        frozen_selection_sha256=engine.file_hash(out/'final_selection.json'),
...
def save_freeze(out,freeze):
    ...
    if path.exists():
        ...
        return saved
    freeze['frozen_at']=engine.utc_now();engine.atomic_json(path,freeze);return freeze
...
def execute(output=OUTPUT,workers=4):
    ...
    hard_pass_ids=sorted(hard.loc[hard.candidate_id.isin(new_ids)&hard.measured_pass&hard.complete_period,'candidate_id'].tolist())
    if not hard_pass_ids:
        inputs=['baseline_reproduction.csv','bottleneck.csv','candidates.json','references.json','phase_memberships.csv']
        freeze=dict(status='NO_ELIGIBLE_CANDIDATE',formal_status='BLOCK_SUBMISSION',candidate_id=None,
            candidate_count=len(candidates),previous_candidate_count=1087,reference_ids=[r['candidate_id'] for r in references],
            hard_pass_ids=[],stop_reason='ALL_NEW_FAILED_REQUIRED_DEVELOPMENT_EPISODE',downstream_evaluation='NOT_RUN',
            training_period=ctx['study']['splits']['development'],validation_period=ctx['study']['splits']['validation'],
            prior_holdout_exposure=True,code_commit=json.loads((out/'study_manifest.json').read_text())['git_head'],
            objective=ctx['study']['objective'],selection_input_sha256={name:engine.file_hash(out/name) for name in inputs})
        freeze=save_freeze(out,freeze);cfg=engine._candidate_config(spec())
        engine.atomic_json(out/'competition_24d_candidate.json',dict(config=cfg,selection=freeze,config_sha256=engine.digest(cfg)))
        seal(out,freeze);return
    screened=hardrank[hardrank.candidate_id.isin(hard_pass_ids)].candidate_id.head(12).tolist()
    ...
    freeze=dict(status='SELECTED_RESEARCH_ONLY' if selected else 'NO_ELIGIBLE_CANDIDATE',formal_status='BLOCK_SUBMISSION',
        ...
        candidate_count=len(candidates),previous_candidate_count=1087,prior_holdout_exposure=True,
        reference_ids=[r['candidate_id'] for r in references],hard_pass_ids=hard_pass_ids,downstream_evaluation='COMPLETED',
        training_period=ctx['study']['splits']['development'],validation_period=ctx['study']['splits']['validation'],
