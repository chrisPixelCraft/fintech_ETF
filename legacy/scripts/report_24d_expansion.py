...
def render(out):
    ...
    from scripts.verify_24d_expansion import verify
    ...
    lines = ["# v3：追加參數搜尋", "",
             f'本輪新增 **{len(candidates)} 組參數**，不重複前輪 {selection["previous_candidate_count"]} 組；另重跑兩組既有對照。結論為 **`{selection["status"]}`**；正式提交狀態為 **`{selection["formal_status"]}`**。開發期代表只是診斷對象，不等於合格候選。', "",
    ...
    lines += [f"本輪代表在驗證期通過 {int(current.passed)}/{int(current.attempted)}，前輪代表為 {int(previous.passed)}/{int(previous.attempted)}；條件式 median 分別為 {pct(current.median_24d_return)}、{pct(previous.median_24d_return)}。這些結果沒有滿足全窗口通過門檻，不能宣稱取得可採用的新最佳策略。", ""]
    lines += ["完整 mean、worst MDD、turnover、形成 20 檔持股的天數與通過率見[比較 CSV](24d_expansion_comparison.csv)。", "",
              "**共同通過窗口的 Median 24D**", "",
              "| 期間 | 共同窗口 | " + " | ".join(labels) + " |",
              "|---|---:|" + "---:|" * len(labels)]
    for phase, title in PERIODS.items():
    ...
    lines += ["", "共同樣本仍排除了失敗窗口，只能用於條件式診斷，不能代表全部起始日期的實際表現；[共同樣本 CSV](24d_expansion_common.csv)保留 P25、P10 與分母。", "",
              "## 搜尋與採用", "",
    ...
    descriptions = {"uniform": "規格範圍均勻抽樣", "availability": "偏向放寬進場篩選", "joint_local": "前輪開發前二局部網格"}
    lines += ["| 搜尋家族 | 新增參數 |", "|---|---:|"]
    for family, group in memberships.groupby("family", sort=False):
        lines.append(f"| {descriptions[family]} | {group.candidate_id.nunique()} |")
    lines += ["", "家族皆使用原規格範圍；調整進場篩選不放寬任何帳務或合規規則。固定 seed 為 2409202602，只使用前輪開發排名作局部搜尋起點。這是有界抽樣與局部組合，並非窮舉全部參數。", "",
              "| 階段 | 參數組數 | 窗口嘗試 | 通過窗口 |", "|---|---:|---:|---:|"]
    ...
    cash_count_round = int(reasons.eq("FAIL_CASH;FAIL_HOLDING_COUNT;FAIL_ROUND_LOT").sum())
    cash_count = int(reasons.eq("FAIL_CASH;FAIL_HOLDING_COUNT").sum())
    odd_holdings = 0
    ...
    lines += ["**困難窗口的失敗分類**", "",
              "| 失敗原因 | 新參數組數 |", "|---|---:|",
              f"| 僅零股持倉門檻 | {round_only} |",
              f"| 現金、檔數、零股持倉 | {cash_count_round} |",
              f"| 現金、檔數 | {cash_count} |", "",
              f"新增 {len(new_hard)} 組中，{int(new_hard.complete_period.sum())} 組跑完窗口，{round_only} 組沒有原始現金／檔數／權重等超限日，但仍未通過保守的零股持倉門檻。逐筆帳本中，全部成交量皆為 1,000 股倍數；{odd_holdings} 組有零股持倉殘餘。公司行動後的零股持倉，不等於送出了零股委託；官方處理契約未明，這輪保留不通過判定。", "",
              ...
              "| 對照 | 參數設定 |", "|---|---|"]
    for label, candidate in labels.items():
        lines.append(f"| {label} | [`{candidate}`](../outputs/24d_expansion/configs/{candidate}.json) |")
    lines += ["", "[凍結紀錄](../outputs/24d_expansion/final_selection.json)與[搜尋設定](../config/24d_expansion_study.json)保存搜尋邊界、採用條件與來源；[驗證排名](../outputs/24d_expansion/validation_ranking.csv)保留全部入圍結果。", "",
              "## 近期滾動壓力", "",
    ...
    lines += ["", "最近 126 個完整滾動起點僅供凍結後壓力測試，重疊窗口不是獨立樣本，報酬仍只計完整且通過窗口。", "",
              ...
              f'已驗證 {verified["groups"]} 組帳本、{verified["attempts"]} 次窗口嘗試。詳見[驗證紀錄](../outputs/24d_expansion/verification.json)與[SHA256 清單](../outputs/24d_expansion/result_manifest.json)。', ""]
    return "\n".join(lines), table, common


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=ROOT / "outputs/24d_expansion")
    parser.add_argument("--verify", action="store_true")
    args = parser.parse_args()
    prose, table, common = render(args.output)
    targets = {
        ROOT / "reports/24d_expansion.md": prose,
        ROOT / "reports/24d_expansion_comparison.csv": table.to_csv(index=False),
        ROOT / "reports/24d_expansion_common.csv": common.to_csv(index=False),
    }
