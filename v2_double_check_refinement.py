"""Audited third-round research replay; official policy submission is blocked."""
...
STUDY = ROOT / "outputs/v2_double_check_refinement"
PARENT_STUDY = ROOT / "outputs/v2_double_check_expansion"
OLDER_STUDY = ROOT / "outputs/v2_double_check_fintuned"
...
def verify_release():
    from scripts.verify_double_check_refinement import verify_release as verify
    return verify(STUDY)
...
def run_backtest(track, output):
    ...
    for protected in (STUDY.resolve(), PARENT_STUDY.resolve(), OLDER_STUDY.resolve()):
    ...
    with tempfile.TemporaryDirectory(dir=target.parent, prefix=".refinement-replay-") as temp:
