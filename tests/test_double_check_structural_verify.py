"""Independent coverage and claim guards for the fourth-round structural grid."""
...
from scripts import verify_double_check_structural as verify
...
class StructuralVerificationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.parent_audit = verify.read(
            verify.PRIOR_OUTPUT / "official_ex_post/final/refinement_selected/independent_audit.json")
    ...
    def test_frozen_grid_rejects_missing_or_substituted_candidate(self):
        ...
        changed = copy.deepcopy(self.study)
        changed["candidates"][0]["params"]["target_count"] = 31
        with self.assertRaises(ValueError):
            verify.validate_grid(changed, self.parent, self.old_candidates)
        changed = copy.deepcopy(self.study)
        changed["axes"]["ema_pair"] = [[5, 20], [10, 30], [20, 50]]
    ...
    def test_high_return_with_one_breach_cannot_displace_incumbent(self):
        ...
        record = dict(track="official_ex_post", candidate_id="z0000",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            selection = self._selection("z0000", record["params"], 100.)
