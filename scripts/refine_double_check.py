"""Frozen, resumable four-axis refinement of the audited double-check expansion.
...
PARENT_ROOT = ROOT / 'outputs/v2_double_check_expansion'
...
PARENT_FINAL = 'expansion_selected'
FINAL_NAME = 'refinement_selected'
RUN_OWNED = False
...
def _parent_audit():
    from scripts.verify_double_check_expansion import verify_release
    ...
    if (audit.get('status') != 'PASS' or audit.get('selected_candidate') != 'x0352'
            ...
            or audit.get('verified_trials') != 1146):
        raise ValueError('Parent release is not the expected audited expansion')
    for track in TRACKS:
...
def make_study():
    """Resolve the exact 81 positions against all 1,409 prior configurations."""
    _parent_audit()
    ...
    prior = {}
    for trial in first_manifest['study']['candidates'] + first_local['candidates'] + expansion['candidates']:
    ...
    for grid_index, combination in enumerate(itertools.product(*AXES.values())):
        ...
        if key in prior:
            ...
            candidate_id, source = f'y{len(trials):04d}', 'new'
    ...
    if (len(grid), len(added), len(trials), sum(r['reused_prior'] for r in grid)) != (81, 80, 80, 1):
        raise ValueError('Frozen refinement counts changed')
    ...
    return dict(study_id='v2_double_check_refinement_20260923',
                parent_study_id='v2_double_check_expansion_20260923',
...
def prepare():
    if STUDY_PATH.exists():
        raise FileExistsError('Frozen refinement design exists: ' + str(STUDY_PATH))
    study = make_study()
...
def dependencies():
    ...
    for relative in ('scripts/refine_double_check.py',
                     'config/v2_double_check_refinement.json',
                     'outputs/v2_double_check_expansion/audit.json',
                     'outputs/v2_double_check_expansion/selection.json',
                     'outputs/v2_double_check_expansion/grid.json',
                     'outputs/v2_double_check_expansion/manifest.json',
                     'outputs/v2_double_check_expansion/study_audit.json'):
...
def _finalize(root, study, rows):
    ...
    for track in TRACKS:
        ...
        comparison.append(dict(track=track, model='refinement_selected', **result['metrics']))
        ...
        monthly += [dict(track=track, model='refinement_selected', **r)
...
def run(args):
    ...
    if root.exists():
        if not args.resume or not manifest_path.is_file():
            raise FileExistsError('Use --resume only for a valid unfinished refinement')
        manifest = json.loads(manifest_path.read_text())
        if manifest['outputs_complete']:
            raise ValueError('Completed refinement is immutable')
        ...
        if manifest['study_sha256'] != base_runner.sha(STUDY_PATH) or manifest['study'] != study:
            raise ValueError('Frozen refinement study changed')
    else:
    ...
    rows = base_runner.batch(study['candidates'], root, args.workers, 'refinement')
    if (len(rows) != 2 * len(study['candidates'])
            or len({canonical(t['params']) for t in study['candidates']}) != 80):
        raise ValueError('Refinement is incomplete or duplicated')
...
def main():
    ...
    parser.add_argument('--output', default='outputs/v2_double_check_refinement')
