"""Independently verify the bounded double-check structural and freeze its release.
...
from scripts import verify_double_check_refinement as parent_release
...
DEFAULT_OUTPUT = ROOT / "outputs/v2_double_check_structural"
PRIOR_OUTPUT = ROOT / "outputs/v2_double_check_refinement"
FOUNDATION_OUTPUT = ROOT / "outputs/v2_double_check_fintuned"
EXPANSION_OUTPUT = ROOT / "outputs/v2_double_check_expansion"
PARENT_RELEASE = "refinement_selected"
...
def _prior_evidence(study):
    ...
    require(prior.sha(PRIOR_OUTPUT / "audit.json") == study["parent_audit_sha256"],
            "Prior release audit differs from the structural's frozen baseline")
    require(old_audit["selected_candidate"] == study["parent_candidate_id"] == "x0352",
            "Structural parent differs from independently audited prior winner")
    ...
    expansion_manifest = read(EXPANSION_OUTPUT / "manifest.json")
    old_trials = (foundation_manifest["study"]["candidates"] +
                  foundation_grid["candidates"] +
                  expansion_manifest["study"]["candidates"] +
                  old_manifest["study"]["candidates"])
    by_signature = {prior.signature(trial["params"]): trial for trial in old_trials}
    require(len(old_trials) == len(by_signature) == 1489, "Prior candidate coverage differs")
...
def validate_grid(study, prior_selected, old_by_signature):
    ...
    require(isinstance(axes, dict) and len(axes) == 4 and all(
        ...
        "Structural needs four declared, nonempty axes")
    require(axes == {"target_count": [20, 25, 30],
                     "cash_guard_ratio": [0.08, 0.12, 0.16],
                     "return_pair": [[5, 20], [10, 30], [15, 40]],
                     "ema_pair": [[5, 20], [10, 30], [15, 40]]},
            "Structural axes differ from frozen four-axis protocol")
    combinations = itertools.product(*axes.values())
    rows = study["grid"]
    require(len(rows) == 81, "Structural grid must contain 81 positions")
    candidates = study["candidates"]
    require(len(candidates) == 80, "Structural must contain 80 new effective candidates")
    require(study.get("raw_grid_combinations") == study.get("effective_grid_combinations") == 81
            ...
            "Structural study misstates coverage or research scope")
    ...
    for index, (values, row) in enumerate(zip(combinations, rows)):
        ...
        require(row["grid_index"] == index, "Structural grid index/order differs")
        prior.same(row["values"], wanted_values, "Structural grid values")
        ...
        for axis, value in wanted_values.items():
            if axis in prior.PAIR_KEYS:
                ...
                require(axis in params, "Unexpected structural parameter axis: " + axis)
                params[axis] = value
        from src.official_deep_tuning import validate_params
        validate_params(params)
        signature = prior.signature(params)
        require(signature not in seen, "Duplicate effective structural tuple")
        seen.add(signature)
        ...
        if older is not None:
            ...
            candidate_id = f"z{len(expected_new):04d}"
    ...
    require(reused == 1 and len(expected_new) == 80 and
            ...
            "Structural deduplication/coverage differs")
    return candidates
...
def _verify_selection(root, study, records, old_selected):
    ...
    require(selection.get("candidate_id") == winner["candidate_id"],
            "Structural winner differs from independently audited rank")
...
def build_audit(root, workers):
    ...
    prior.same(read(root / "grid.json"), dict(
        ...
        new_candidate_count=80, parent_candidate_id="x0352"), "Saved structural grid")
    ...
    inputs = set(manifest["input_hashes"]) | {
        "scripts/verify_double_check_structural.py", "scripts/verify_double_check_refinement.py",
        "scripts/verify_double_check_expansion.py",
        "scripts/verify_double_check_release.py",
        "scripts/report_double_check_structural.py", "tests/test_double_check_structural_verify.py",
        "v2_double_check_structural.py",
        ...
        "reports/v2_double_check_structural_report.md",
        "docs/v2_double_check_structural_protocol.md",
    }
    for pattern in ("reports/v2_double_check_structural*", "docs/v2_double_check_structural*",
                    "v2_double_check_structural*.py"):
    ...
    audit = dict(status="PASS", scope="INDEPENDENT_RESEARCH_ACCOUNTING_AND_BOUNDED_STRUCTURAL_ONLY",
...
def verify_release(root):
    ...
    require(audit.get("status") == "PASS" and audit.get("submission_status") == "BLOCK_SUBMISSION"
            ...
            and audit.get("scope") == "INDEPENDENT_RESEARCH_ACCOUNTING_AND_BOUNDED_STRUCTURAL_ONLY"
            ...
            "Structural release status/scope differs")
    require(audit.get("auditor_sha256") == prior.sha(Path(__file__)),
            "Structural release verifier source changed")
    prior.verify_hashes(ROOT, audit["input_hashes"],
                        {"scripts/verify_double_check_structural.py", "config/v2_double_check_structural.json",
                         "scripts/search_double_check_structural.py", "scripts/report_double_check_structural.py",
                         "tests/test_double_check_structural_verify.py",
                         "v2_double_check_structural.py",
                         "reports/v2_double_check_structural_report.md",
                         "docs/v2_double_check_structural_protocol.md",
                         "outputs/v2_double_check_refinement/audit.json"})
    ...
    require(manifest["study_sha256"] == prior.sha(STUDY_PATH)
            ...
            "Structural design or prior baseline changed")
    ...
    require(audit.get("selected_candidate") == read(root / "selection.json")["candidate_id"]
            ...
            "Structural release winner differs")
    ...
    require(status.get("status") == "TRUSTED_RESEARCH_ONLY_BLOCK_SUBMISSION"
            ...
            "Structural release status or audit hash differs")
    return audit
