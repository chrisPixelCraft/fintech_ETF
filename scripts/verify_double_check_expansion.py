"""Independently verify the bounded double-check expansion and freeze its release.
...
DEFAULT_OUTPUT = ROOT / "outputs/v2_double_check_expansion"
PRIOR_OUTPUT = ROOT / "outputs/v2_double_check_fintuned"
RELEASE = "expansion_selected"
TRACKS = prior.TRACKS
TABLES = prior.TABLES
...
def _prior_evidence(study):
    old_audit = prior.verify_release(PRIOR_OUTPUT)
    require(prior.sha(PRIOR_OUTPUT / "audit.json") == study["parent_audit_sha256"],
            "Prior release audit differs from the expansion's frozen baseline")
    require(old_audit["selected_candidate"] == study["parent_candidate_id"] == "d0034",
            "Expansion parent differs from independently audited prior winner")
    selected = read(PRIOR_OUTPUT / "selection.json")
    old_manifest = read(PRIOR_OUTPUT / "manifest.json")
    old_grid = read(PRIOR_OUTPUT / "local_grid.json")
    old_trials = old_manifest["study"]["candidates"] + old_grid["candidates"]
    by_signature = {prior.signature(trial["params"]): trial for trial in old_trials}
    require(len(old_trials) == len(by_signature) == 836, "Prior candidate coverage differs")
    require(selected["candidate_id"] == "d0034" and selected["params"] ==
            next(t["params"] for t in old_trials if t["candidate_id"] == "d0034"),
            "Prior parent parameters differ")
    for track in TRACKS:
        folder = PRIOR_OUTPUT / track / "final" / prior.RELEASE
...
def validate_grid(study, prior_selected, old_by_signature):
    ...
    axes = study["axes"]
    require(isinstance(axes, dict) and len(axes) == 6 and all(
        isinstance(values, list) and values for values in axes.values()),
        "Expansion needs six declared, nonempty axes")
    combinations = itertools.product(*axes.values())
    rows = study["grid"]
    require(len(rows) == 576, "Expansion grid must contain 576 positions")
    candidates = study["candidates"]
    require(len(candidates) == 573, "Expansion must contain 573 new effective candidates")
    require(study.get("raw_grid_combinations") == study.get("effective_grid_combinations") == 576
            and study.get("reused_parent_count") == 3
            and study.get("new_candidate_count") == 573
            ...
            "Expansion study misstates coverage or research scope")
    ...
    for index, (values, row) in enumerate(zip(combinations, rows)):
        ...
        require(row["grid_index"] == index, "Expansion grid index/order differs")
        prior.same(row["values"], wanted_values, "Expansion grid values")
        ...
        for axis, value in wanted_values.items():
            if axis in prior.PAIR_KEYS:
                ...
                require(axis in params, "Unexpected expansion parameter axis: " + axis)
                params[axis] = value
        from src.official_deep_tuning import validate_params
        validate_params(params)
        signature = prior.signature(params)
        require(signature not in seen, "Duplicate effective expansion tuple")
        seen.add(signature)
        ...
        if older is not None:
            ...
            candidate_id = f"x{len(expected_new):04d}"
            ...
            require(trial.get("phase") == "exhaustive_six_axis_grid"
                    and trial.get("varied") == "joint6", "Unexpected trial phase")
            expected_new[candidate_id] = trial
    require(reused == 3 and len(expected_new) == 573 and
            set(expected_new) == set(candidate_by_id),
            "Expansion deduplication/coverage differs")
    return candidates
...
def _incumbent_record(old_selected):
    folder = PRIOR_OUTPUT / "official_ex_post/final" / prior.RELEASE
    audit = read(folder / "independent_audit.json")
    return dict(track="official_ex_post", candidate_id="d0034",
...
def _verify_selection(root, study, records, old_selected):
    ...
    require(selection.get("candidate_id") == winner["candidate_id"],
            "Expansion winner differs from independently audited rank")
    ...
    source = "prior" if winner["candidate_id"] == "d0034" else "new"
    ...
    require(selection.get("incumbent_candidate_id") == "d0034"
...
def _verify_final(root, selection, records):
    ...
    for track in TRACKS:
        ...
        expected_folder = ((PRIOR_OUTPUT / track / "final" / prior.RELEASE) if source == "prior"
...
def _verify_reported_tables(root, results):
    ...
    for track in TRACKS:
        ...
        parent = read(PRIOR_OUTPUT / track / "final" / prior.RELEASE / "metrics.json")
        comparison.append(dict(track=track, model="parent_d0034", **parent))
...
def build_audit(root, workers):
    ...
    prior.same(read(root / "grid.json"), dict(
        ...
        new_candidate_count=573, parent_candidate_id="d0034"), "Saved expansion grid")
    ...
    require(len(expected) == manifest.get("completed_trials") == manifest.get("expected_trials") == 1146,
            "Incomplete expansion trial count")
    for track in TRACKS:
        actual = {p.name for p in (root / track / "trials").iterdir() if p.is_dir()}
        require(actual == {t["candidate_id"] for t in candidates},
                "Missing or unexpected expansion trial directories")
    ...
    require(study_audit.get("status") == "PASS" and study_audit.get("verified_trials") == len(records)
            ...
            and study_audit.get("grid_raw") == 576
            and study_audit.get("new_effective") == 573
            and study_audit.get("parent_reused") == 3
    ...
    inputs = set(manifest["input_hashes"]) | {
        "scripts/verify_double_check_expansion.py", "scripts/verify_double_check_release.py",
        "scripts/report_double_check_expansion.py", "tests/test_double_check_expansion_verify.py",
        "v2_double_check_expansion.py",
        "scripts/check_double_check_prefix.py", "tests/test_double_check_causality.py",
        "outputs/v2_double_check_fintuned/audit.json",
        "reports/v2_double_check_expansion_report.md",
        "docs/v2_double_check_expansion_protocol.md",
    }
    for pattern in ("reports/v2_double_check_expansion*", "docs/v2_double_check_expansion*",
                    "v2_double_check_expansion*.py"):
    ...
    audit = dict(status="PASS", scope="INDEPENDENT_RESEARCH_ACCOUNTING_AND_BOUNDED_EXPANSION_ONLY",
                 ...
                 verified_trials=len(records), raw_grid_combinations=576,
                 new_effective_candidates=573, reused_prior_candidates=3,
...
def verify_release(root):
    ...
    require(audit.get("status") == "PASS" and audit.get("submission_status") == "BLOCK_SUBMISSION"
            ...
            and audit.get("verified_trials") == 1146,
            "Expansion release status/scope differs")
    require(audit.get("auditor_sha256") == prior.sha(Path(__file__)),
            "Expansion release verifier source changed")
    prior.verify_hashes(ROOT, audit["input_hashes"],
                        {"scripts/verify_double_check_expansion.py", "config/v2_double_check_expansion.json",
                         "scripts/expand_double_check.py", "scripts/report_double_check_expansion.py",
                         "tests/test_double_check_expansion_verify.py",
                         "v2_double_check_expansion.py",
                         "reports/v2_double_check_expansion_report.md",
                         "docs/v2_double_check_expansion_protocol.md",
                         "outputs/v2_double_check_fintuned/audit.json"})
    ...
    require(manifest.get("outputs_complete") is True
            and manifest.get("completed_trials") == manifest.get("expected_trials") == 1146,
            "Expansion manifest incomplete")
    require(all(audit["input_hashes"].get(path) == digest
                for path, digest in manifest["input_hashes"].items()),
            "Expansion release omits frozen study input")
    study = read(STUDY_PATH)
    require(manifest["study_sha256"] == prior.sha(STUDY_PATH)
            ...
            "Expansion design or prior baseline changed")
    old = prior.verify_release(PRIOR_OUTPUT)
    require(old["selected_candidate"] == study["parent_candidate_id"] == "d0034",
