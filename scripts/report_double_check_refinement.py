...
OUTPUT = ROOT / "outputs/v2_double_check_refinement"
PARENT = ROOT / "outputs/v2_double_check_expansion"
REPORT = ROOT / "reports/v2_double_check_refinement_report.md"
STUDY = ROOT / "config/v2_double_check_refinement.json"
TRACKS = ("official_ex_post", "historical_pit")
FINAL = "refinement_selected"
...
def make_report(root=OUTPUT, report=REPORT):
    ...
    for track in TRACKS:
        ...
        by_track[track] = rows
    parent_folder = PARENT / "official_ex_post/final/expansion_selected"
    ...
    lines = [
        ...
        "[稽核檔](../outputs/v2_double_check_refinement/audit.json)。Active Share、官方帳本與平台收件"
        "證據不能靠回測補足。", "",
        f"本輪完整覆蓋四軸 81 個位置，1 組沿用已稽核的 `x0352`；80 組新設定各在兩池回放，"
        f"共 160 次新試驗。連同前兩輪累計 1,489 組不同設定。官方事後池有 {len(passed)}／80 "
    ...
    for track in TRACKS:
        old = read(PARENT / track / "final/expansion_selected/metrics.json")
        new = final[track][1]
        for label, row in (("原 x0352", old), (selected["candidate_id"], new)):
    ...
    for axis in study["axes"]:
        lines.append(f"| `{axis}` | {selected['params'][axis]} |")
    lines += ["", "[四軸細搜規格](../docs/v2_double_check_refinement_protocol.md)以外的參數固定於 `x0352`。"
              ...
              "[試驗表](../outputs/v2_double_check_refinement/trials.csv) · "
              "[網格位置](../outputs/v2_double_check_refinement/grid.json) · "
              "[獲選設定](../outputs/v2_double_check_refinement/selection.json) · "
              "[月度淨值](../outputs/v2_double_check_refinement/monthly.csv) · "
