"""Audited expansion research replay; official policy submission remains blocked."""
...
STUDY = ROOT / 'outputs/v2_double_check_expansion'
PRIOR_STUDY = ROOT / 'outputs/v2_double_check_fintuned'
...
def verify_release():
    from scripts.verify_double_check_expansion import verify_release as verify
    return verify(STUDY)
...
def run_backtest(track, output):
    ...
    for protected in (STUDY.resolve(), PRIOR_STUDY.resolve()):
    ...
    with tempfile.TemporaryDirectory(dir=target.parent, prefix='.expansion-replay-') as temp:
