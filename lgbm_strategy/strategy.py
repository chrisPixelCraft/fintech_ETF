"""LightGBMStrategy: AsOfView -> features -> LightGBM predicted return -> portfolio.target_weights().
...
every decision day. Scores are the predicted forward returns of the names that
are feature-ready at D-1.
"""
...
STRATEGY_KEYS = {'horizon', 'lookback', 'refit_every', 'portfolio'}


@dataclass(frozen=True)
class LightGBMStrategyConfig:
    horizon: int = 10          # label: forward return over t+1..t+horizon
...
def predict_scores(fitted: model.FittedModel, view: AsOfView) -> pd.Series:
    """Predicted forward return per feature-ready symbol at ``view.date``, sorted by symbol."""
...
class LightGBMStrategy:
    ...
    def decide(self, view: AsOfView, state: PortfolioState) -> dict:
        ...
        if self.fitted is None or state.day_index % c.refit_every == 0:
            data = build_training_set(view, c.horizon, c.lookback)
