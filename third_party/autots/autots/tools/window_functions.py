...
def chunk_reshape(
    ...
    if num_chunks < 1:
        num_chunks = 1
    # fintech_ETF patch P1: upstream used `num_series % num_chunks`, which drops
    # series (and leaves np.empty rows uninitialized) whenever num_series > chunk_size
    # and is not a multiple of chunk_size, e.g. 150 series. The first chunk must absorb
    # every series beyond num_chunks * chunk_size.
    excess_series = max(num_series - num_chunks * chunk_size, 0)
